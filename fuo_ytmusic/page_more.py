...
class UploadingSongListModel(QAbstractTableModel):
    ...
    def update_state(self, index: int, state: bool):
        ...
        self.dataChanged.emit(
            self.index(index, 0), self.index(index, len(self._header) - 1)
        )
...
class FavRenderer(Renderer, LibraryTabRendererMixin):
    ...
    def _load_uploader(self):
        ...
        self._uploader.setSource(
            QUrl.fromLocalFile(
                (Path(__file__).parent / "qml" / "uploader.qml").as_posix()
            )
        )
    ...
    async def render(self):
        ...
        if self.tab_id == Tab.songs:
            ...
            self.show_songs(
                await aio.run_fn(lambda: self._provider.library_upload_songs())
            )
            self.toolbar.show()
            ...
            self.show_artists(
                await aio.run_fn(lambda: self._provider.library_upload_artists())
            )
        elif self.tab_id == Tab.albums:
            self.show_albums(
                await aio.run_fn(lambda: self._provider.library_upload_albums())
            )
