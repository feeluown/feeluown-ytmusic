...
class ExploreBackend(QObject):
        ...
        result = [
            {
                "key": category.key,
                "value": [
                    {"title": playlist.title, "params": playlist.params}
                    for playlist in category.value
                ],
            }
    ...
    async def playlists(self, params: str):
        ...
        playlists = await loop.run_in_executor(
            None, self._provider.service.category_playlists, params
        )
        result = [
            {"id": p.playlistId, "name": p.title, "cover": p.thumbnail}
            for p in playlists
        ]
...
class ExploreRenderer(TabBarRendererMixin):
    ...
    async def render(self):
        ...
        view.rootContext().setContextProperty(
            "flow_index", self.get_flow_index(self.tab_index)
        )
        ...
        view.setSource(
            QUrl.fromLocalFile(
                (Path(__file__).parent / "qml" / "page_explore.qml").as_posix()
            )
        )
    ...
    def render_by_tab_index(self, tab_index):
        self._app.browser.goto(
            page="/providers/ytmusic/explore",
