    ...
    def load_user_cookies(self):
        if HEADER_FILE.exists():
            with HEADER_FILE.open('r') as f:
                data = json.load(f)
                if 'Cookie' in data:
                    return self._parse_text_cookies(data['Cookie'])
                # If the content in HEADER_FILE is generated by ytmusicapi, just accept it.
                # ytmusicapi oauth can generate a file named 'oauth.json'.
                # User can rename the 'oauth.json' to HEADER_FILE.
                return data
